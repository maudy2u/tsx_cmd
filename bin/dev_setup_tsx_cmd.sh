...
if [ "$(uname)" == "Darwin" ]; then
 ...
 if [ "$(uname -p)" == "aarch64" ]; then
   ${METEOR} add ostrio:files@=1.6.0 flemay:less-autoprefixer semantic:ui@=2.2.6_5 jquery react-meteor-data froatsnook:sleep package-stats-opt-out akasha:shelljs session vsivsi:job-collection dburles:collection-helpers ostrio:logger ostrio:loggerconsole ostrio:loggerfile ostrio:meteor-root ostrio:loggermongo
